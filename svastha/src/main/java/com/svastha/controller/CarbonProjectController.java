...
public class CarbonProjectController {
	...
	@Autowired
	private AwdDataRepository awdDataDao;
	...
	public @ResponseBody CarbonAwdMapping getCarbonProjects(@RequestParam Long projectId) {
	...
	public @ResponseBody CarbonAwdMapping saveCarbonProject(@RequestBody CarbonAwdMapping carbonAwdMapping) {
		return carbonAwdMappingDao.save(carbonAwdMapping);
	}

	@GetMapping("/awdDatas")
	public @ResponseBody Page<AwdData> getAwdData(@RequestParam Long projectId, Pageable pageable) {
		FarmProjects project = projectDao.findById(projectId)
				.orElseThrow(() -> new ResourceNotFoundException("Project not found with id " + projectId));

		CarbonAwdMapping carbonAwdMapping = carbonAwdMappingDao.findAllByProjects(project);

		if (carbonAwdMapping == null) {
			return Page.empty();
		}
		AwdDevice device = carbonAwdMapping.getDevice();
		String deviceId = device.getDeviceId();
		return awdDataDao.findAllByDeviceId(deviceId, pageable);
	}
